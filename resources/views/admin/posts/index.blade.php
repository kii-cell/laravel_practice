                            ...
                            @foreach ($posts as $val)
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">
                                        {{ optional($val->published_at)->format('Y-m-d H:i:s') ?? '未定' }}
                                    </td>
                                    <td class="border px-4 py-2 flex items-center justify-center space-x-2">
                                        <a href="{{ route('admin.posts.show', $val->id) }}"
                                            class="text-blue-600 hover:underline">詳細</a>
                                        {{-- 編集ボタンを追加 --}}
                                        <a href="{{ route('admin.posts.edit', $val->id) }}"
                                            class="ml-2 text-green-600 hover:underline">編集</a>
                                        {{-- 削除ボタンの追加 --}}
                                        <form action="{{ route('admin.posts.delete', $val->id) }}" method="POST"
